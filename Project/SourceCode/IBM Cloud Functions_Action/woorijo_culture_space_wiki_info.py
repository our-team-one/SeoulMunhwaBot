...
class CulturalFacilityInfo: # 구 이름을 받으면 그 구에 속한 문화공간 정보를 반환하는 클래스
    ...
    # 테마 명소의 제목만 모아서 반환 (중복제거)
    def getSeoulThemeDataList(self, number_of_samples):

        theme_info = set([])
        theme_info_list = []
        random_items = []

        #Festival_get_url = "https://korean.visitseoul.net/event-festival?curPage="
        ThemaCourse_get_url = "https://korean.visitseoul.net/hallyu-themetour?curPage="
        #FestivalData's last_page = 3
        ThemaCourse_last_page = 2
        ...
        #for i in range(1, ThemaCourse_last_page):
        res = requests.get(ThemaCourse_get_url + str(1)).text
        html = bs(res, "html.parser")
        container = html.find("div", class_ = "container-inner")
        li_list = container.parent.findAll("li")

        for item in li_list:
            title = item.find("div", class_ = "lst-infor-title")
            title = re.sub(cleaner, '', str(title))

            theme_info.add(title)

        #theme_info_list = list(theme_info)

        for row in list(theme_info):
            row = row.replace('&lt;','')
            row = row.replace('&gt;','')
            theme_info_list.append(row)

        # list(theme_info)
        # 무작위로 10개 추출
        if len(theme_info_list) > 10:
            random_items = random.choices(population=theme_info_list, k=number_of_samples)
        else:
            random_items = theme_info_list
    ...
    #     for i in range(1, ThemaCourse_last_page):
    #         res = requests.get(ThemaCourse_get_url + str(i)).text
    #         html = bs(res, "html.parser")
    #         container = html.find("div", class_ = "container-inner")
    #         li_list = container.parent.findAll("li")
    ...
    # 테마 명소의 제목에 맞는 제목, 이미지, 링크 반환
    def getSeoulThemeDataListByTitle(self, search_word, height, width):
        #Festival_get_url = "https://korean.visitseoul.net/event-festival?curPage="
        ThemaCourse_get_url = "https://korean.visitseoul.net/hallyu-themetour?curPage="
        #FestivalData's last_page = 3
        ThemaCourse_last_page = 2
        ...
        for item in li_list:
            ...
            if title == search_word:
                ...
                img_src = base_url + item.find("img").get("src")
                #temp_list = [title, link, img_src]
                line = title +  "<a href = '"  + link + "'>" + "<img src = '" +img_src + "' height =" + str(height) + " width = "+ str(width) + "></a>"
                theme_info.append(line)
    ...
    # 축제 명소의 제목만 모아서 반환 (중복제거)
    def getSeoulFestiDataList(self, number_of_samples):
        ...
        #for i in range(1, FestivalData_last_page):
        res = requests.get(Festival_get_url + str(1)).text
        html = bs(res, "html.parser")
        container = html.find("div", class_ = "container-inner")
        li_list = container.parent.findAll("li")
        ...
        for item in li_list:
            ...
            festi_info.add(title)

        #theme_info_list = list(theme_info)

        for row in list(festi_info):
            row = row.replace('&lt;','')
            row = row.replace('&gt;','')
            festi_info_list.append(row)

        # list(theme_info)
        # 무작위로 10개 추출
        if len(festi_info_list) > 10:
            random_items = random.choices(population=festi_info_list, k=number_of_samples)
        else:
            random_items = festi_info_list
    ...
    #     for i in range(1, FestivalData_last_page):
    #         res = requests.get(Festival_get_url + str(i)).text
    #         html = bs(res, "html.parser")
    #         container = html.find("div", class_ = "container-inner")
    #         li_list = container.parent.findAll("li")
    ...
    # 테마 명소의 제목에 맞는 제목, 이미지, 링크 반환
    def getSeoulFestiDataListByTitle(self, search_word, height, width):
        Festival_get_url = "https://korean.visitseoul.net/event-festival?curPage="
        #ThemaCourse_get_url = "https://korean.visitseoul.net/hallyu-themetour?curPage="
        #FestivalData_last_page = 3
        #ThemaCourse_last_page = 2
    ...
    # def getVisitSeoulData(get_url, last_page):
    #     #Festival_get_url = "https://korean.visitseoul.net/event-festival?curPage="
    #     #ThemaCourse_get_url = "https://korean.visitseoul.net/hallyu-themetour?curPage="
    #     #FestivalData's last_page = 3
    #     #ThemaCourseData's last_page = 2
    ...
    #         res = requests.get(get_url + str(i)).text
    #         html = bs(res, "html.parser")
    #         container = html.find("div", class_ = "container-inner")
    #         li_list = container.parent.findAll("li")
...
def main(dict):
    ...
    # 서울시 테마코스 data 집합중 제목 10개 가져오기
    if dict.get("node") == "thema" :
        ...
        final_result = cfi.getSeoulThemeDataList(10)
    ...
    if dict.get("node") == "thema_info" :
        ...
        final_result = cfi.getSeoulThemeDataListByTitle(title, 100, 100)
    ...
    if dict.get("node") == "festival" :
        ...
        final_result = cfi.getSeoulFestiDataList(10)
    ...
    # festival_info
    if dict.get("node") == "festival_info" :
        ...
        final_result = cfi.getSeoulFestiDataListByTitle(title, 100, 100)
